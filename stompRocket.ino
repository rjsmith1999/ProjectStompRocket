...
void loop()
{
  ...
  height.f = getHeight();
  sendbuffer[5] = height.i;

  union floatInt_t test;
  test.f = 3.14159;
  sendbuffer[6] = test.i; //This can be used as a test to confirm proper transmission;
...
void getIMUData(int* sendbuffer)
{
  ...
  {  
    ...
    converter.f = (float)myIMU.accelCount[0]*myIMU.aRes; // - accelBias[0];
    sendbuffer[2] = converter.i;
    converter.f = (float)myIMU.accelCount[1]*myIMU.aRes; // - accelBias[1];
    sendbuffer[3] = converter.i;
    converter.f = (float)myIMU.accelCount[2]*myIMU.aRes; // - accelBias[2];
